...
* Request    | Descrição                                                    *
* --------------------------------------------------------------------------*
* TBDK921296 | AGIR - Extração de Dados para Migração - Imobilizado         *
* --------------------------------------------------------------------------*
REPORT zrag_imobilizado_v2.
...
SELECT-OPTIONS : so_emp   FOR anla-bukrs, " Empresa
                 so_imo   FOR anla-anln1, " Nº principal do imobilizado
                 so_class FOR anla-anlkl, " Classe do imobilizado
                 so_gjahr FOR anlc-gjahr, " Exercício
                 so_deakt FOR anla-deakt. " Data de desativação

PARAMETERS:
  p_brdatu TYPE rbada-brdatu,              "Data de relatório de um report
  p_afabe1 TYPE rbada-afabe1 DEFAULT '01'. "Primeira área de avaliação (reporting)

SELECTION-SCREEN END OF BLOCK b1.

SELECTION-SCREEN BEGIN OF BLOCK b2 WITH FRAME TITLE text-t00.
PARAMETERS:
  p_maxreg TYPE /sapcnd/code_line_num DEFAULT 0,
...
**************************************
  ...
  TRY.
      ...
      "Selecionar as Ã¡reas de depreciaÃ§ao
...
* Inicio AlteraÃ§Ã£o - MA004818 Marcelo Alvares - 15.01.2019 16:12:16
         z~bdatu GE @sy-datum AND "AtribuiÃ§Ãµes de imobilizado com data valor erro Retorna duplicidade
         a~deakt IN @so_deakt.
* Fim alteraÃ§Ã£o - MA004818 Marcelo Alvares - 15.01.2019 16:12:21
...
*&---------------------------------------------------------------------*
*   Selecionar as Ã¡reas de depreciaÃ§Ã£o
*----------------------------------------------------------------------*
  ...
  TRY.
      ...
      CASE 'X'.
        ...
        WHEN rb_down.
          ...
          IF sy-batch IS NOT INITIAL. " Verifica se Ã© background

            MESSAGE 'NÃ£o Ã© possivel baixar o arquivo pois a execuÃ§Ã£o Ã© em background' TYPE 'S' DISPLAY LIKE 'E'.
...
*&       --> InstÃ¢ncia Excel
*&      --> P_ABA       Nome da aba
*&      --> P_TI_TABLE  Tabela com os dados extraÃ­dos
*&---------------------------------------------------------------------*
...
FORM display_online USING p_rawdata     TYPE solix_tab
  ...
  IF sy-batch IS NOT INITIAL. " Verifica se Ã© background
    ...
    MESSAGE ID 'FES' TYPE 'S' NUMBER '002' DISPLAY LIKE 'E'.
    MESSAGE 'NÃ£o Ã© exibido o resultado em tela pois a execuÃ§Ã£o Ã© em background' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
