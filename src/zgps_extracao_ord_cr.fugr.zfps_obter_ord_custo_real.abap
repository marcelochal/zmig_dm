...
* Módulo...........: PS                                                     *
* Programa.........: ZFPS_OBTER_ORD_CUSTO_REAL                              *
* Transação........:                                                        *
* Tipo de Prg......: FUNÇÃO                                                 *
* Objetivo.........: Obter os custos reais para Ordens (KOB1)               *
* Data.............: 16/08/2018                                             *
* --------------------------------------------------------------------------*
* Request    | Descrição                                                    *
* --------------------------------------------------------------------------*
...
FUNCTION zfps_obter_ord_custo_real.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"     REFERENCE(P_I_SO_DATA_LANC) TYPE  ISU_RANGES_DATE
*"     REFERENCE(P_I_SO_TPORD) TYPE  MNT_T_AUART_RANGE
*"     REFERENCE(P_I_NUM_LINHAS) TYPE  /SAPCND/CODE_LINE_NUM
*"  EXPORTING
*"     REFERENCE(P_E_TI_EXT_ORD_CR) TYPE  ZTPS_EXT_ORD_CUSTO_REAL
*"  RAISING
...
*"----------------------------------------------------------------------
  ...
  DATA: lc_ordem_validada TYPE char01,
        ...
        ti_ext_ord_cr     TYPE ztps_ext_ord_custo_real,
        wa_op_sel         LIKE LINE  OF ti_op_sel,
        wa_ext_ord_cr     TYPE zeps_ext_ord_custo_real,
...
*--------------------------------------------------*
* Data
  IF NOT p_i_so_data_lanc[] IS INITIAL.

    READ TABLE p_i_so_data_lanc INTO wa_rg_data INDEX 1.
...
*--------------------------------------------------------*
* Isso faz a lógica Standard do Report RKAEP000 (KOB1)
...
*    EXPORTING
*      p_i_nome_prog_standad = 'RKAEP000'. "Nome do programa Standard que será chamado (KOB1)

*--------------------------------------------------*
* Buscar as Ordens: partidas individuais reais (KOB1)
  SUBMIT rkaep000
  WITH SELECTION-TABLE ti_op_sel
  AND RETURN.

* Os dados do ALV (KOB1) serão carregados aqui
...
*--------------------------------------------------*
  IF <lt_data> IS ASSIGNED.
    ...
    LOOP AT <lt_data> ASSIGNING <ls_data>.
      ...
      MOVE-CORRESPONDING <ls_data> TO wa_ext_ord_cr.

      CALL FUNCTION 'ZFPS_VALIDAR_ORD_CR'
        EXPORTING
          p_i_ordem          = wa_ext_ord_cr-aufnr
          p_i_empresa        = wa_ext_ord_cr-bukrs
          p_i_codigo_dc      = wa_ext_ord_cr-beknz
      ...
      IF lc_ordem_validada EQ abap_true.
        ...
        APPEND wa_ext_ord_cr TO ti_ext_ord_cr.
        CLEAR  wa_ext_ord_cr.
...
*--------------------------------------------------*
  IF NOT ti_ext_ord_cr[] IS INITIAL.
    p_e_ti_ext_ord_cr[] = ti_ext_ord_cr[].
  ENDIF.
