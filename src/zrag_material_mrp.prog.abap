*&---------------------------------------------------------------------*
*& Report ZRAG_MATERIAL_MRP
*&---------------------------------------------------------------------*
*&  Programa de Extração MM - MESTRE MATERIAL - VISAO MRP
...
DATA:
  ...
  wa_mat                   TYPE ty_mat,
  li_pos                   TYPE i,
  ti_layout_mrp_cockpit TYPE STANDARD TABLE OF ZEMM_EXT_MAT_MRP,
  wa_layout_mrp_cockpit TYPE ZEMM_EXT_MAT_MRP.
...
*** Seleciona o universo de materiais a serem extraidos
  ...
  TRY .
      ...
      IF  ti_mat_exp[] IS NOT INITIAL.
        ...
        SELECT matnr werks beskz eisbe eislo
          FROM marc
            UP TO p_maxreg ROWS
      ...
      MOVE-CORRESPONDING ti_marc TO ti_layout_mrp_cockpit.

** Monta dados

      FIELD-SYMBOLS: <fs_mat> TYPE ZEMM_EXT_MAT_MRP.

      SORT ti_mat_exp BY matnr.

      LOOP AT ti_layout_mrp_cockpit ASSIGNING <fs_mat>.
      ...
      IF p_local IS NOT INITIAL.
        cl_salv_table=>factory( IMPORTING r_salv_table = lo_table CHANGING t_table = ti_layout_mrp_cockpit ).
        lo_table->display( ).
      ...
      ELSE.
        ...
        ASSIGN ti_layout_mrp_cockpit TO <t_tab>.
        EXPORT:
