...
*&---------------------------------------------------------------------*
*&  Programa de Extração MM - MESTRE MATERIAL - VISAO COMPRAS
*&  Programador: Wilson Perotta Jose
*&---------------------------------------------------------------------*
REPORT ZRAG_MATERIAL_COMPRAS.
...
DATA:
  ...
  wa_mara                  TYPE mara,
  li_pos                   TYPE i,
  ti_layout_compras_cockpit TYPE STANDARD TABLE OF ZEMM_EXT_MAT_COMPRAS,
  wa_layout_compras_cockpit TYPE ZEMM_EXT_MAT_COMPRAS.
...
*** Seleciona o universo de materiais a serem extraidos
  ...
  TRY .
      ...
      IF  ti_mat_exp[] IS NOT INITIAL.
        ...
        SELECT matnr werks steuc indus
          FROM marc
            UP TO p_maxreg ROWS
          INTO CORRESPONDING FIELDS OF TABLE ti_marc
        ...
        IF ti_marc[] IS NOT INITIAL.
          ...
          SELECT matnr bstme mfrpn mfrnr
            FROM mara
      ...
      MOVE-CORRESPONDING ti_marc TO ti_layout_compras_cockpit.

** Monta dados da mara

      FIELD-SYMBOLS: <fs_mat> TYPE ZEMM_EXT_MAT_COMPRAS.

      SORT ti_mara BY matnr.
      SORT ti_mat_exp BY matnr.

      LOOP AT ti_layout_compras_cockpit ASSIGNING <fs_mat>.

        READ TABLE ti_mara INTO wa_mara WITH KEY matnr = <fs_mat>-matnr BINARY SEARCH.
        IF sy-subrc = 0.
            <fs_mat>-bstme  = wa_mara-bstme.
            <fs_mat>-mfrpn  = wa_mara-mfrpn.
            <fs_mat>-mfrnr  = wa_mara-mfrnr.
        ENDIF.
      ...
      IF p_local IS NOT INITIAL.
        cl_salv_table=>factory( IMPORTING r_salv_table = lo_table CHANGING t_table = ti_layout_compras_cockpit ).
        lo_table->display( ).
      ...
      ELSE.
        ...
        ASSIGN ti_layout_compras_cockpit TO <t_tab>.
        EXPORT:
