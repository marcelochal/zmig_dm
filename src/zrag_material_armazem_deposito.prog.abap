*&---------------------------------------------------------------------*
*& Report ZRAG_MATERIAL_ARMAZEM_DEPOOSITO
*&---------------------------------------------------------------------*
*&  Programa de Extração MM - MESTRE MATERIAL - VISAO ARMAZEM DEPOSITO
*&  Programador: Wilson Perotta Jose
*&---------------------------------------------------------------------*
REPORT ZRAG_MATERIAL_ARMAZEM_DEPOSITO.
...
DATA:
  ...
  wa_mat                   TYPE ty_mat,
  li_pos                   TYPE i,
  ti_layout_armazemdep_cockpit TYPE STANDARD TABLE OF ZEMM_EXT_MAT_ARMAZEM_DEPOSITO,
  wa_layout_armazemdep_cockpit TYPE ZEMM_EXT_MAT_ARMAZEM_DEPOSITO.

FIELD-SYMBOLS <t_tab>  TYPE STANDARD TABLE.

SELECTION-SCREEN BEGIN OF BLOCK b1 WITH FRAME TITLE text-t00.
SELECT-OPTIONS : so_werks  FOR gc_werks NO INTERVALS OBLIGATORY.
SELECT-OPTIONS : so_lgort  FOR gc_lgort MATCHCODE OBJECT H_T001L NO INTERVALS OBLIGATORY.
...
*** Seleciona o universo de materiais a serem extraidos
  ...
  TRY .
      ...
      MOVE-CORRESPONDING ti_mard TO ti_layout_armazemdep_cockpit.


** Monta dados da mara

      FIELD-SYMBOLS: <fs_mat> TYPE ZEMM_EXT_MAT_ARMAZEM_DEPOSITO.

      SORT ti_mat_exp BY matnr.

      LOOP AT ti_layout_armazemdep_cockpit ASSIGNING <fs_mat>.
      ...
      IF p_local IS NOT INITIAL.
        cl_salv_table=>factory( IMPORTING r_salv_table = lo_table CHANGING t_table = ti_layout_armazemdep_cockpit ).
        lo_table->display( ).
      ...
      ELSE.
        ...
        ASSIGN ti_layout_armazemdep_cockpit TO <t_tab>.
        EXPORT:
