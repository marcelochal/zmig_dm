class ZCL_EXTRATOR_INFORMACOES_PM definition
  public
  create public .
...
private section.
  ...
  methods GET_CABECALHO_PLANO .
  methods GET_PLANO_OPERACAO .
  methods GET_LOCAL_INSTALACAO .
...
CLASS ZCL_EXTRATOR_INFORMACOES_PM IMPLEMENTATION.
  ...
  METHOD exporta_dados.
    ...
    FREE: t_equi, t_equz, t_crhd, t_iloa, t_plko, t_plpo, t_iflot, t_iflotx. CLEAR lv_programa.
  ENDMETHOD.
  ...
  METHOD exporta_equipamentos.
    LOOP AT t_equi ASSIGNING FIELD-SYMBOL(<fs_equi>).
      ...
      <fs_equip_excel>-erdat     = <fs_equi>-erdat.
  ...
  METHOD exporta_lista_tarefas.
    LOOP AT t_plko ASSIGNING FIELD-SYMBOL(<fs_plko>).
      LOOP AT t_plpo ASSIGNING FIELD-SYMBOL(<fs_plpo>) WHERE plnnr EQ <fs_plko>-plnnr.
  ...
  METHOD exporta_local_install.
    LOOP AT t_equi ASSIGNING FIELD-SYMBOL(<fs_equi>).
      ...
      <fs_loc_inst_excel>-eqart = <fs_equi>-eqart.

      READ TABLE t_equz ASSIGNING FIELD-SYMBOL(<fs_equz>) WITH KEY equnr = <fs_equi>-equnr BINARY SEARCH.
      IF <fs_equz> IS ASSIGNED.
        <fs_loc_inst_excel>-iwerk = <fs_equz>-iwerk.
        <fs_loc_inst_excel>-ingrp = <fs_equz>-ingrp.
        ...
        IF <fs_iloa> IS ASSIGNED.
          ...
          READ TABLE t_iflot ASSIGNING FIELD-SYMBOL(<fs_iflot>) WITH KEY tplnr = <fs_iloa>-tplnr BINARY SEARCH.
          IF <fs_iflot> IS ASSIGNED.
      ...
      ENDIF.
    ENDLOOP.
    UNASSIGN: <fs_loc_inst_excel>, <fs_equi>, <fs_equz>, <fs_crhd>, <fs_iloa>, <fs_iflot>, <fs_iflotx>.
  ...
  METHOD get_cabecalho_plano.
    ...
    CLEAR: lv_maxreg. FREE r_tip_roteiro. SORT t_plko BY plnty plnnr.
  ENDMETHOD.
  ...
  METHOD get_cab_cent_trab.
    ...
    ELSEIF lv_programa EQ c_tab_valores-prog_pla_man.
      IF NOT t_plan_manut IS INITIAL.
        ...
        WHERE objid EQ @t_plan_manut-gewrk.
      ENDIF.
  ...
  METHOD get_int_equipam.
    IF NOT t_equi IS INITIAL.
      SELECT equnr, datbi, equzn, iwerk, mapar, hequi, heqnr, ingrp, gewrk, iloan, rbnr
      INTO TABLE @t_equz
      FROM equz
      FOR ALL ENTRIES IN @t_equi
      WHERE equnr EQ @t_equi-equnr AND
            datbi EQ @c_tab_valores-data_val.
      SORT t_equz BY equnr datbi.
    ENDIF.
  ENDMETHOD.


  METHOD get_local_instalacao.
    IF NOT t_iloa IS INITIAL.
      SELECT tplnr, tplkz, fltyp, tplma, lgwid
      INTO TABLE @t_iflot
      FROM iflot
      FOR ALL ENTRIES IN @t_iloa
      WHERE tplnr EQ @t_iloa-tplnr.
  ...
  METHOD get_plano_operacao.
    IF NOT t_plko IS INITIAL.
      SELECT plnty, plnnr, zaehl, vornr, steus, arbid, werks, ltxa1, vplal, plifz, preis, peinh, waers, dauno, daune, arbei, arbeh,
             anzzl, prznt, slwid, usr00, indet, larnt, bmeih, bmvrg, aufkt, equnr
      INTO TABLE @t_plpo
      FROM plpo
      FOR ALL ENTRIES IN @t_plko
      WHERE plnnr EQ @t_plko-plnnr.
      SORT t_plpo BY plnty plnnr.
    ENDIF.
  ENDMETHOD.
