*&---------------------------------------------------------------------*
*& Report ZRAG_FORNECEDOR_IMPOSTO RET
*&---------------------------------------------------------------------*
*&  Programa de Extração MM - MESTRE FORNECEDOR - IMPOSTO RETIDO
*&  Programador: Wilson Perotta Jose
*&---------------------------------------------------------------------*

REPORT ZRAG_FORNECEDOR_IMPOSTORET.
...
DATA:
  ...
  ti_lfbw                  TYPE TABLE OF lfbw,
  ti_fornec_imp            TYPE TABLE OF ty_fornec,
  ti_fornec_exp            TYPE TABLE OF ty_fornec,
  wa_fornec                TYPE ty_fornec,
  li_pos                   TYPE i,
  ti_layout_fornec_cockpit TYPE STANDARD TABLE OF ZEMM_EXT_MAT_FORNEC_IMPOSTORET,
  wa_layout_fornec_cockpit TYPE ZEMM_EXT_MAT_FORNEC_IMPOSTORET.
...
SELECT-OPTIONS : so_witht  FOR gc_witht NO INTERVALS OBLIGATORY.
...
CALL FUNCTION 'ZFMM_EXTRACAO_SELECIONA_FORNEC'
  ...
  TRY .
      ...
      IF  ti_fornec_exp[] IS NOT INITIAL.
        ...
        SELECT *
         ...
         WHERE lifnr = ti_fornec_exp-lifnr
           ...
           AND witht IN so_witht.

      ENDIF.

      IF ti_lfbw[] IS INITIAL.
      ...
      MOVE-CORRESPONDING ti_lfbw TO ti_layout_fornec_cockpit.

** Monta dados da mara

      FIELD-SYMBOLS: <fs_for> TYPE ZEMM_EXT_MAT_FORNEC_IMPOSTORET.
